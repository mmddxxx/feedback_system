...
import com.example.barrierfree_feedback_system.common.utils.MD5Utils;
import com.example.barrierfree_feedback_system.common.vo.ResStatus;
import com.example.barrierfree_feedback_system.common.vo.Result;
import com.example.barrierfree_feedback_system.mapper.dao.UsersMapper;
    ...
    public Result userResgit(String name, String pwd) {
        synchronized (this) {
            ...
            if (users.size() == 0) {
                ...
                if (i > 0) {
                    return new Result(ResStatus.OK, "注册成功！", user);
                } else {
                    return new Result(ResStatus.NO, "注册失败！", null);
                }
            } else {
                return new Result(ResStatus.NO, "用户名已经被注册！", null);
            }
        }
    }
...
//
        ...
        if(users.size() == 0){
            return new Result(ResStatus.NO,"登录失败，用户名不存在！",null);
        }else{
            String md5Pwd = MD5Utils.md5(pwd);
